...
- (int) renderButtons
{
    ...
    for (int i = 0; i < self.namesArrays.count+1; i++) {
        ...
        [button setTitleColor:[UIColor colorWithWhite:0.434 alpha:1.000] forState:UIControlStateNormal];
...
- (void)buttonClick:(UIButton *)sender
{
    ...
    }else if ([self.delegate respondsToSelector:@selector(buttonMenuCXCustemSheet:didSelectedAtIndex:)]){
        
        [self.delegate buttonMenuCXCustemSheet:self didSelectedAtIndex:sender.tag];
    }
    
    self.selectedAtIndex(sender.tag);
    [self hide];
}
